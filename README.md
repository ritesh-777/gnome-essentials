...
Shelf mode also includes a Capture Current Workspace action. This acts as a **system-level "save state" for your mind**: it captures your active window layout as a Workspace Profile and saves it as a unified workspace item on the Shelf. Crucially, the extension uses heuristic title matching to automatically identify the specific file, URL, or directory open inside each application window. For apps whose open file can be inferred confidently (such as terminals, file managers, office suites, PDF readers, and text/code editors), GNOME Essentials automatically attaches that document, path, or directory context under the matching application. Clicking the captured workspace item later launches the entire set of apps, opens their respective project files, and restores their exact tiling layouts, monitors, and screen positions with a single action. If the file or folder cannot be identified confidently, the app is still captured without a guessed attachment.
